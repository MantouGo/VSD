...
#p6.setAlignment(QtCore.Qt.AlignVCenter)
...
b1,a1   = signal.butter(6,[40.0/nqy , 200.0/nqy], 'band') #heart rate parameter for fft 過濾器的順序 || 臨界頻率 || 過濾器的類型 Default為'lowpass'
b, a    = signal.butter(6, [6.0/nqy , 30.0/nqy],'band') #breath rate parameter for fft 過濾器的順序 || 臨界頻率 || 過濾器的類型 Default為'lowpass'
...
def vtsExec():
	...
	if dck:
		...
		#print(len(rangeBuf))
		rp7 = [np.sqrt(rangeBuf[i*2]*rangeBuf[i*2] + rangeBuf[i*2+1]*rangeBuf[i*2+1]) for i in range(19)] # array: 必須確定其平方根的輸入值。 out: [可選]放置結果的備用array對象。 返回: 返回array中數字的平方根。
